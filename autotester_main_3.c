...
//basic join dead lock error test
//==============================================================================
pthread_t tid_t8;
static int yunk;
static void* _thread_deadlock_test(void* arg) {

    int r = pthread_join(tid_t8, NULL);

    if(r == EDEADLK) yunk = 10;
...
static int test8(void) {
    ...
    pthread_create(&tid_t8, NULL, &_thread_deadlock_test, NULL);

    pthread_join(tid_t8, NULL);
    if(yunk == 10) return PASS;

    return FAIL;
}

//basic multiple thread join error test
//==============================================================================

pthread_t tid_t9_1, tid_t9_2, tid_t9_3;
static int wait2, wait1;
static void* _multi_thread_join_1(void* arg) {
    
    while(wait2!=10) {};
    wait1 = 20;
    pthread_join(tid_t9_2, NULL);
    
    pthread_exit(0);
}
static void* _multi_thread_join_2(void* arg) {
    wait2 = 10;
...
static int test9(void) {
    ...
    pthread_create(&tid_t9_1, NULL, &_multi_thread_join_1, NULL);
    pthread_create(&tid_t9_2, NULL, &_multi_thread_join_2, NULL);

    while(wait1!=20) {};

    int r = pthread_join(tid_t9_2, NULL);
    if(r == EINVAL)
        return PASS;
...
static int test10(void) {
    int result;
    sem_t sema;
    result = sem_init(&sema, 0, 100000);
    if(result==EINVAL) return PASS;
    return FAIL;
}


// destroy non existent semaphore error
//==============================================================================
...
#define NUM_TESTS 12
static int (*test_arr[NUM_TESTS])(void) = {&test0, &test1, &test2, &test3, &test4, &test5, &test6, &test7, &test8, &test9, &test10, &test11}; //, &test6, &test7
